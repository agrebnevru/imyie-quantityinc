...
class CIMYIEQuantityInc
{
    ...
    public static function OnAfterIBlockElementAddHandler(&$arFields): void
    {
        ...
        if ((int)$ELEMENT_ID > 0 && (int)$IBLOCK_ID > 0 && CModule::IncludeModule(
                'iblock'
            ) && CModule::IncludeModule('catalog')) {
            $arProduct = CCatalogProduct::GetByID($ELEMENT_ID);
            if (!$arProduct) {
                $arCatalog = CCatalog::GetByID($IBLOCK_ID);
                if (0 < (int)$arCatalog["OFFERS_IBLOCK_ID"]) {
    ...
    public static function OnBeforeIBlockElementUpdateHandler(&$arFields): void
    {
        $ELEMENT_ID = $arFields["ID"];
        $IBLOCK_ID = $arFields["IBLOCK_ID"];
        if ((int)$ELEMENT_ID > 0 && (int)$IBLOCK_ID > 0 && CModule::IncludeModule(
    ...
    public static function OnProductAddHandler($ID, $arFields): void
    {
        $PRODUCT_ID = $ID;
        if ((int)$PRODUCT_ID > 0 && CModule::IncludeModule('iblock') && CModule::IncludeModule('catalog')) {
    ...
    public static function OnBeforeProductUpdateHandler($ID, &$arFields): void
    {
        $PRODUCT_ID = $ID;
        if ((int)$PRODUCT_ID > 0 && CModule::IncludeModule('iblock') && CModule::IncludeModule('catalog')) {
            $srElement = CIBlockElement::GetByID($PRODUCT_ID);
