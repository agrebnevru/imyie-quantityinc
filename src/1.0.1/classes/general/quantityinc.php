...
class CIMYIEQuantityInc
{
	...
	public static function OnAfterIBlockElementAddHandler(&$arFields) {
		...
		if(IntVal($ELEMENT_ID)>0 && IntVal($IBLOCK_ID)>0 && CModule::IncludeModule('iblock') && CModule::IncludeModule('catalog')) {
	...
	public static function OnBeforeIBlockElementUpdateHandler(&$arFields) {
		...
		if(IntVal($ELEMENT_ID)>0 && IntVal($IBLOCK_ID)>0 && CModule::IncludeModule('iblock') && CModule::IncludeModule('catalog')) {
			...
			if(is_array($PROP_CACHE) && count($PROP_CACHE)<1) {
				$resPr = CIBlockElement::GetProperty($IBLOCK_ID,$ELEMENT_ID,array(),array("CODE"=>self::$PROPERTY_CODE_INT));
				if($arProperty = $resPr->GetNext()) {
					$PROP_CACHE[$arProperty["ID"]] = $arProperty["VALUE"];
				}
				$resPr = CIBlockElement::GetProperty($IBLOCK_ID,$ELEMENT_ID,array(),array("CODE"=>self::$PROPERTY_CODE_DATETIME));
				if($arProperty = $resPr->GetNext()) {
					$PROP_CACHE[$arProperty["ID"]] = $arProperty["VALUE"];
				}
			}
			...
			if($arElement = $srElement->GetNext()) {
				...
				if(IntVal($ELEMENT_ID)>0 && IntVal($IBLOCK_ID)>0) {
					...
					if(IntVal($arCatalog["PRODUCT_IBLOCK_ID"])>0) {
						...
						$dbRes = CIBlockElement::GetProperty($IBLOCK_ID,$ELEMENT_ID,array(),array("ID"=>$arCatalog["SKU_PROPERTY_ID"]));
	...
	public static function _SetPropertyValue($ELEMENT_ID,$IBLOCK_ID) {
		$time = time();
		$arIDs = self::CheckProperties($IBLOCK_ID);
		CIBlockElement::SetPropertyValues($ELEMENT_ID,$IBLOCK_ID,$time,$arIDs["PROPERTY_CODE_INT"]);
		CIBlockElement::SetPropertyValues($ELEMENT_ID,$IBLOCK_ID,ConvertTimeStamp($time,"FULL"),$arIDs["PROPERTY_CODE_DATETIME"]);
		\Bitrix\Iblock\PropertyIndex\Manager::updateElementIndex($IBLOCK_ID,$ELEMENT_ID);
	}
	
	public static function CheckProperties($IBLOCK_ID,$PROPERTY_CODE_INT="",$PROPERTY_CODE_DATETIME="") {
		$return = array();
		...
		// int property
		$dbProperties = CIBlockProperty::GetList(array("sort"=>"asc","name"=>"asc"),array("IBLOCK_ID"=>$IBLOCK_ID,"CODE"=>$PROPERTY_CODE_INT));
		...
		// date_time property
		$dbProperties = CIBlockProperty::GetList(array("sort"=>"asc","name"=>"asc"),array("IBLOCK_ID"=>$IBLOCK_ID,"CODE"=>$PROPERTY_CODE_DATETIME));
		if(!$arFields = $dbProperties->GetNext()) {
			// add property
			$arFields = Array(
				"IBLOCK_ID" => $IBLOCK_ID,
